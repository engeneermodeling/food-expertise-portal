...
export async function getCourseData(locale: string): Promise<CourseWithLessons> {
  try {
    const coursePath = path.join(contentDirectory, locale, 'courses', 'food-safety-management');
...
export async function getLessonData(locale: string, number: string): Promise<LessonData> {
  const lessonNumber = parseInt(number);
  const lessonPath = path.join(contentDirectory, locale, 'courses', 'food-safety-management', `topic-${number}.mdx`);
